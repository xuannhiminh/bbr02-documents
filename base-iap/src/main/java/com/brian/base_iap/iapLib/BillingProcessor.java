 ...
 */
package com.brian.base_iap.iapLib;
...
public class BillingProcessor extends BillingBase
{
	...
	private static final long ACK_RETRY_DELAY_MS = 1000L;
	...
	public interface ISkuDetailsResponseListenerV7
	{
	...
	**/
						...
						if (purchaseItem.getPurchaseState() == Purchase.PurchaseState.PURCHASED
							...
							AcknowledgePurchaseParams ackParams =
									AcknowledgePurchaseParams.newBuilder()
											.setPurchaseToken(purchaseItem.getPurchaseToken())
											.build();
									...
									new AcknowledgePurchaseResponseListener() {
										...
										public void onAcknowledgePurchaseResponse(@NonNull BillingResult result) {
											if (result.getResponseCode()
												...
												Log.d(LOG_TAG, "Auto‑acked restored purchase: "
														+ purchaseItem.getPurchaseTime());
											} else {
												Log.e(LOG_TAG, "Failed to ack restored purchase: "
	...
	private void loadSubByTypeAsyncV7(final BillingCache cacheStorage,
										  final IPurchasesResponseListener listener)
	{
		if (!isConnected())
		{
		...
		QueryPurchasesParams params = QueryPurchasesParams.newBuilder()
				.setProductType(Constants.PRODUCT_TYPE_SUBSCRIPTION)
				.build();

		billingService.queryPurchasesAsync(params, new PurchasesResponseListener()
		{
			@Override
			public void onQueryPurchasesResponse(@NonNull BillingResult billingResult,
												 @NonNull List<Purchase> list)
	...
	public void loadOwnedSubFromGoogleAsyncV7(final IPurchasesResponseListener listener)
	{
		final IPurchasesResponseListener successListener = new IPurchasesResponseListener()
		{
			@Override
		...
		loadSubByTypeAsyncV7(cachedSubscriptions, successListener);
	}
	...
	/**
	 * Launches a purchase flow for the given subscription product and basePlan.
	 *
	 * @param activity    your Activity
	 * @param productId   the subscription Product ID (e.g. "sub_premium")
	 * @param basePlanId  the Base Plan ID to subscribe (e.g. "test-yearly-plan" or "test-monthly-plan")
	 * @return true if the flow was started, false on immediate failure
	 */
	public boolean subscribeV7(
			final Activity activity,
			@NonNull final String productId,
			@NonNull final String basePlanId) {

		if (!isConnected() || TextUtils.isEmpty(productId) || TextUtils.isEmpty(basePlanId)) {
			if (!isConnected()) retryBillingClientConnection();
			return false;
		}

		// Save payload
		String payload = Constants.PRODUCT_TYPE_SUBSCRIPTION + ":" + productId;
		savePurchasePayload(payload);

		// Query ProductDetails
		List<QueryProductDetailsParams.Product> productList = new ArrayList<>(1);
		productList.add(
		...
		billingService.queryProductDetailsAsync(params, new ProductDetailsResponseListener() {
			@Override
			public void onProductDetailsResponse(
					@NonNull BillingResult billingResult,
					@NonNull List<ProductDetails> detailsList) {
				...
				// Find the SubscriptionOfferDetails matching the requested basePlanId
				List<ProductDetails.SubscriptionOfferDetails> offers = pd.getSubscriptionOfferDetails();
				if (offers == null || offers.isEmpty()) {
					reportBillingError(
							Constants.BILLING_ERROR_FAILED_TO_INITIALIZE_PURCHASE,
							new Throwable("No subscription offers available")
					);
					return;
				}
				...
				if (selectedOffer == null) {
					reportBillingError(
							Constants.BILLING_ERROR_FAILED_TO_INITIALIZE_PURCHASE,
							new Throwable("Base plan not found: " + basePlanId)
					);
					return;
				}
				...
				// Build and launch the billing flow
				BillingFlowParams.ProductDetailsParams prodParams =
				...
				BillingFlowParams flowParams = BillingFlowParams.newBuilder()
						.setProductDetailsParamsList(Collections.singletonList(prodParams))
						.build();

				int responseCode = billingService.launchBillingFlow(activity, flowParams).getResponseCode();
	...
	private void getSkuDetailsAsyncV7(final String productId, String purchaseType,
									final ISkuDetailsResponseListenerV7 listener)
	{
		ArrayList<String> productIdList = new ArrayList<>();
		productIdList.add(productId);
		getSkuDetailsAsyncV7(productIdList, purchaseType, new ISkuDetailsResponseListenerV7()
		{
			@Override
			public void onSkuDetailsResponse(@Nullable List<ProductDetails> products)
			{
				if (products != null)
				{
					if (listener != null)
					{
						reportSkuDetailsResponseCallerV7(products, listener);
					}
				}
			}
			...
			public void onSkuDetailsError(String string)
			{
				reportSkuDetailsErrorCallerV7(string, listener);
			}
		});
	}

	private void getSkuDetailsAsyncV7(final ArrayList<String> productIdList, String purchaseType,
									final ISkuDetailsResponseListenerV7 listener)
	{
		if (billingService == null || !billingService.isReady())
		{
			reportSkuDetailsErrorCallerV7("Failed to call getSkuDetails. Service may not be connected", listener);
			return;
		}
		if (productIdList == null || productIdList.isEmpty())
		{
			reportSkuDetailsErrorCallerV7("Empty products list", listener);
			return;
		}

		try
		{
			List<QueryProductDetailsParams.Product> qpList = new ArrayList<>();
			for (String id : productIdList) {
				qpList.add(QueryProductDetailsParams.Product.newBuilder()
						.setProductId(id)
			...
			QueryProductDetailsParams params =
					QueryProductDetailsParams.newBuilder()
							.setProductList(qpList)
							.build();
			billingService.queryProductDetailsAsync(params, (billingResult, productDetailsList) -> {
				if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {

					listener.onSkuDetailsResponse(productDetailsList);
				} else {
					//callback.onFailure("Billing failed: " + billingResult.getDebugMessage());
					reportSkuDetailsErrorCallerV7("Empty products list", listener);
		...
		catch (Exception e)
		{
			...
			reportSkuDetailsErrorCallerV7(e.getLocalizedMessage(), listener);
		}
	}
	...
	public void getSubscriptionListingDetailsAsyncV7(String productId, ISkuDetailsResponseListenerV7 listener)
	{
		getSkuDetailsAsyncV7(productId, Constants.PRODUCT_TYPE_SUBSCRIPTION, listener);
	}
	...
	private void reportSkuDetailsErrorCallerV7(final String error, final ISkuDetailsResponseListenerV7 listener)
	{
		if (listener != null && handler != null)
		{
			handler.post(() -> listener.onSkuDetailsError(error));
	...
	private void reportSkuDetailsResponseCallerV7(@Nullable final List<ProductDetails> products,
												final ISkuDetailsResponseListenerV7 listener)
	{
		if (listener != null && handler != null)
		{
	...
	private void acknowledgePurchase(Purchase purchase, int retryCount) {
		AcknowledgePurchaseParams ackParams = AcknowledgePurchaseParams.newBuilder()
				.setPurchaseToken(purchase.getPurchaseToken())
				.build();

		billingService.acknowledgePurchase(ackParams, billingResult -> {
			if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
				verifyAndCachePurchase(purchase);
			} else {
				if (retryCount + 1 < MAX_ACK_RETRIES) {
					...
					// give up after MAX_ACK_RETRIES
					reportBillingError(
							Constants.BILLING_ERROR_FAILED_TO_ACKNOWLEDGE_PURCHASE,
							new Exception("Ack failed after retries: " + billingResult.getDebugMessage())
					);
				}
			}
		});
