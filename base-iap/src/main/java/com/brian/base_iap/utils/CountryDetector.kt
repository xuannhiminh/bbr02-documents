...
import com.brian.base_iap.utils.FCMTopicHandler.isDebug
    ...
    private fun getSimCountry(context: Context): String? {
        return try {
            ...
            if (tm.simCountryIso.isNullOrEmpty()) {
                FirebaseAnalytics.getInstance(context).logEvent("no_sim", null)
            }
            tm.simCountryIso?.uppercase(Locale.US)
        } catch (e: Exception) {
            FirebaseAnalytics.getInstance(context).logEvent("no_sim_null", null)
            null
        }
    }
}